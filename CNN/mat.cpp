...
void printmat(float **mat, nSize mat_size){
	// 打印矩阵
	printf("--------------------------------------\n");
	for(int row=0;row<mat_size.r;row++){
		for(int col=0;col<mat_size.c;col++){
			printf("%-8.8f  ",mat[row][col]);
		}
		printf("\n");
	}
	printf("--------------------------------------\n");
...
float** rotate180(float** mat, nSize matSize)// 矩阵翻转180度,相当于上下颠倒且左右颠倒
{
	int i,c,r;
...
float** correlation(float** map,nSize mapSize,float** inputData,nSize inSize,int type)// 互相关
{
	...
	nSize outSize={outSizeH,outSizeW};
...
float** loadmat(const char* filename,nSize matSize)
{
    FILE  *fp=NULL;
    ...
    float ** mat= (float**)malloc(matSize.r*sizeof(float*));
    for(int row=0;row<matSize.c;row++){
        mat[row] = (float*)malloc(matSize.c*sizeof(float));
...
float** loadmat(FILE * fp,nSize matSize, bool closeFile)
{
//    从文件中加载矩阵
    if(fp==NULL)
        printf("open file failed\n");
